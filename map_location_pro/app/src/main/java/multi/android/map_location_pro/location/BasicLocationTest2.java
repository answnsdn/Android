...
//      => gps, network 모듈을 함께 호출해서 먼저 받아지는 Provider를 이용해서 작업할 수 있도록 구현
public class BasicLocationTest2 extends AppCompatActivity implements LocationListener {
    LocationManager locationManager;
    TextView result;
    boolean permission_state;
    List<String> provider_list;//전체 위치제공자 목록
    List<String> enableProvider_list;//사용가능한 위치제공자 목록
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(ContextCompat.checkSelfPermission(this,
            ...
            //권한설정이 완료되면 Provider 목록을 가져와서 출력
            getProviderList();
            getLocation();
        }
    }
    ...
    public void PrintInfo(String provider, Location location){
        ...
        if(location!=null){
            ...
            result.append("Longtitude=>"+location.getLongitude()+"\n");
